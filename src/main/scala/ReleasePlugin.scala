...
object ReleasePlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    object ReleaseKeys {
      ...
      private[this] val WithDefaults: Parser[ParseResult] = (Space ~> "with-defaults") ^^^ ParseResult.WithDefaults
      private[this] val SkipTests: Parser[ParseResult] = (Space ~> "skip-tests") ^^^ ParseResult.SkipTests
      private[this] val CrossBuild: Parser[ParseResult] = (Space ~> "cross") ^^^ ParseResult.CrossBuild
      private[this] val ReleaseVersion: Parser[ParseResult] = (Space ~> "release-version" ~> Space ~> StringBasic) map ParseResult.ReleaseVersion
      private[this] val NextVersion: Parser[ParseResult] = (Space ~> "next-version" ~> Space ~> StringBasic) map ParseResult.NextVersion

      private val FailureCommand = "--failure--"

      private[this] sealed abstract class ParseResult
